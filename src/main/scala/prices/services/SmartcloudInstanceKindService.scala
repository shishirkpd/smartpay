...
object SmartcloudInstanceKindService {
  ...
  final case class Config(baseUri: String, token: String)

  def make[F[_]: Concurrent](config: Config): InstanceKindService[F] = new SmartcloudInstanceKindService(config)

  private final class SmartcloudInstanceKindService[F[_]: Concurrent](config: Config) extends InstanceKindService[F] {
    ...
    override def getPriceByKind(kind: String): F[InstanceKindWithPrice] =
      InstanceKindWithPrice(SmartCloudPriceServiceImpl.getPriceOfInstanceByKind(config.baseUri, config.token, kind))
        .pure[F]
  }
